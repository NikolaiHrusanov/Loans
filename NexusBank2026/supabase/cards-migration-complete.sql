-- =============================================================================
-- NexusBank — Complete Cards migration (idempotent)
-- Run in Supabase Dashboard → SQL Editor AFTER:
--   1. banking-schema.sql
--   2. money-management.sql (profiles + balances)
--   3. profiles-registration-rls.sql (if profile inserts fail)
--
-- This single script creates/updates everything needed for:
--   • Add Existing Card (encrypted PAN, CVV never stored)
--   • NexusBank virtual cards
--   • Card settings, transactions, analytics
--   • RLS tied to auth.users via auth.uid()
-- =============================================================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cards_balance_nonneg'
  ) THEN
    ALTER TABLE public.cards
      ADD CONSTRAINT cards_balance_nonneg CHECK (balance >= 0);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'cards_partner_type_check'
  ) THEN
    ALTER TABLE public.cards
      ADD CONSTRAINT cards_partner_type_check
      CHECK (partner_type IN ('none', 'standard', 'flexi', 'blaze_wear'));
  END IF;
END $$;
...
-- Prevent duplicate external cards for the same user (same network + last4)
CREATE UNIQUE INDEX IF NOT EXISTS cards_external_unique_idx
  ON public.cards (user_id, card_network, last_four_digits)
...
CREATE OR REPLACE FUNCTION public.validate_card_expiry(p_expiry text)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  v text := trim(COALESCE(p_expiry, ''));
  m int;
  y int;
  last_day date;
BEGIN
  IF v !~ '^\d{2}/\d{2}$' THEN
    RETURN false;
  END IF;
  m := substring(v, 1, 2)::int;
  y := 2000 + substring(v, 4, 2)::int;
  IF m < 1 OR m > 12 THEN
    RETURN false;
  END IF;
  last_day := (date_trunc('month', make_date(y, m, 1)) + interval '1 month - 1 day')::date;
  RETURN last_day >= current_date;
END;
$$;

-- ─────────────────────────────────────────────
-- ADD EXISTING CARD (full PAN encrypted; CVV verified then discarded)
-- ─────────────────────────────────────────────
...
-- Legacy helper (last-4 only) — kept for backwards compatibility
...
BEGIN
  ...
  INSERT INTO public.cards (
    ...
    expiry_date, cardholder_name, status, is_virtual, partner_type
  ) VALUES (
    v_user_id,
  ...
  INSERT INTO public.card_settings (card_id, user_id)
...
-- Backfill settings for existing cards
INSERT INTO public.card_settings (card_id, user_id, spending_limit, frozen)
